# -*- coding: utf-8 -*-
"""
ERes2NetV2 声纹模型微调训练脚本 (基于CAM++版改造, 双向margin + 1:1平衡采样 + cmd降噪)
...
与CAM++版的差异:
  ...
  - 冻结: conv1/bn1/layer1/layer2 (ResNet结构, 微调layer3/layer4)
  - 其余(损失/采样/防塌缩/fbank缓存) 与CAM++版完全一致
...
# === cuDNN DLL 修复: 注入 Library/bin 到 PATH (CAM++脚本同样做法) ===
# 解决 'Invalid handle. Cannot load symbol cudnnCreate' (torch找不到配套cudnn DLL)
...
def load_camplus_for_train(device):
    """
    加载 ERes2NetV2 预训练声纹模型 (通过 ModelScope pipeline)

    Args:
        device: str, "cuda" 或 "cpu"
    Returns:
        wrapper: ERes2NetV2 完整模型对象 (含 preprocessing + embedding_model)
...
class FbankCache:
    ...
    def __init__(self, rel_paths, data_root, workers=8):
        ...
        t0 = time.time()
        if workers <= 1:
            # 单进程串行模式 (绕过ProcessPool子进程崩溃问题)
            for i, p in enumerate(rel_paths):
                path, feat = _extract_fbank_one((p, data_root))
